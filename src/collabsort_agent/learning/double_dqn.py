...
Difference from DQN:
  In vanilla DQN, the target is computed as:
      Q_target = r + gamma · max_a' Q_target(s', a')
  This causes an overestimation bias because the same network both selects
  AND evaluates the best next action.

  Double DQN decouples these two steps:
    1. Action selection  → q_network   picks   a* = argmax_a Q_online(s', a)
    2. Action evaluation → target_network evaluates Q_target(s', a*)
  This gives:
...
class DDQN(ActionValueEstimator):
    """Double Deep Q-Learning algorithm for estimating action values.

    Compared to DQN, the only change is in _learn():
    the online network selects the best next action, but its value is read
    from the target network — eliminating the overestimation bias of DQN.
    """
